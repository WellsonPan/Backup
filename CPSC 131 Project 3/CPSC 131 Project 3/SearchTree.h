...
protected:						// local utilities
	...
	SLinkedList<E> pathFinder(const K& k, Position<E>& v, SLinkedList<E>& s);
private: 						// member data
	LinkedBinaryTree<E> T;					// the binary tree
	int n;						// number of entries
	//SLinkedList<E> searchedPath = SLinkedList<E>();
public:
...
template <typename E>					// find utility
SLinkedList<E> SearchTree<E>::pathFinder(const K& k, Position<E>& v, SLinkedList<E>& s) {
	Position<E> l = v.left(), r = v.right();
	if (v.isExternal())
	{
		return s;
	...
	else
	{
		if (k < (*v).key())
		{
			s.addFront(l.getElem());
			return pathFinder(k, l, s);
		}// search left subtree
		else if ((*v).key() < k)
		{
			s.addFront(r.getElem());
			return pathFinder(k, r, s);
		}// search right subtree
		else
		{
			s.addFront(v.getElem());
			return s;
		}// found it here
	}
}
...
SLinkedList<E>* SearchTree<E>::findPath(const K& k) {
	...
	Position<E> v = root();
	SLinkedList<E> *searchPath = new SLinkedList<E>();
	SLinkedList<E> tempPath = pathFinder(k, v, *searchPath);
	SLinkedList<E> *orderedSearchPath = new SLinkedList<E>();
	for (int index = 0; index < tempPath.size(); index++)
	{
		orderedSearchPath->addFront(tempPath.front());
		tempPath.removeFront();
	}
	orderedSearchPath->addFront(root().getElem());
	return orderedSearchPath;
}
