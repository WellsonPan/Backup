...
ChutesAndLaddersGame::ChutesAndLaddersGame(int nPlayers) : winner("no winner") {
	/*for (int index = 0; index < nPlayers; index++)
	{
		...
		cin >> name;
		group.enqueue(Player(name));
	}*/
	group.enqueue(Player("Brenda"));
	group.enqueue(Player("Wellson"));
	gameBoard = GameBoard();
...
void ChutesAndLaddersGame::resetGame() {
	for (int index = 0; index < MIN_NUMBER_OF_PLAYERS; index++)
	{
		group.dequeue();
	}
	group.enqueue(Player("Brenda"));
...
void ChutesAndLaddersGame::playGame() {
	...
	*temp = group.front();
	...
	{
		int newPosition = gameBoard.checkChutesLadders((*temp).rollDieAndMove());
		temp->setPosition(newPosition);
		if (temp->getPosition() == 100)
		{
			winner = temp->getName();
			cout << "Congratulations " << winner << " on winning!" << endl;
			delete temp;
		}
		else
		{
